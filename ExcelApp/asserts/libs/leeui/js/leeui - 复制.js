...
(function ($) {
    ...
    $.leeUI.controls.Grid.leeExtend($.leeUI.core.UIComponent, {
        ...
        */
        ...
        initScrollSetting: function (data) {

            var g = this,
                p = this.options;
            g.offsetLen = 10;//显示留白数据
            var scrollheight = 200;
            var rowHeight = p.rowHeight;
            g.mainLength = Math.ceil(scrollheight / rowHeight);//可视区域
            g.startIndex = 0;
                ...
                if (p.tree && g.collapsedRows && g.collapsedRows.length) {
                    var isHide = function () {
                        ...
                        while (pitem) {
                            ...
                            if (pitem["__hideChild"]) return true;
        ...
        _getHtmlFromData: function (data, frozen) {
            ...
            var g = this,
                p = this.options;
            if (!g.offsetLen) g.initScrollSetting(data);
            ...
            var end = g.endIndex + res.middle + res.start;
            if (end > data.length) end = data.length;
            for (var i = g.startIndex + res.start; i < end; i++) {
                var item = data[i];


                var rowid = item['__id'];
                if (!item) continue;
                if (g.isRowHide(item)) continue;
                gridhtmlarr.push('<tr');
                ...
                } else if (g.isSelected(item) || item["__selected"]) {
                    gridhtmlarr.push(' lee-selected');
            ...
            console.log("g.totalHeight"+g.totalHeight+"g.endIndex:" + g.endIndex + "res end:" + res.end);
        ...
        _getTreeCellHtml: function (oldContent, rowdata) {
            ...
            if (rowdata["__hideChild"]) isExtend = false;
